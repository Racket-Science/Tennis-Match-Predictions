...
def prepare_atp():
    '''
    This function reads .csv files fort ATP match data for the open era (1968-2022), cleans the columns, limits data to 1999-2019, 
    drops records that were decided to not add significant value to overall data, '''
    # read .csv into df if available
    if os.path.isfile('ATP_matches.csv'):
        df = pd.read_csv('ATP_matches.csv', index_col = 0)
    ...
    # otherwise, prep that 
        ...
        for subset in subsets:
            ...
            # locate indexes where there are mmtls
        ...
        df['player1_wins'] = np.where(df['winner'] == df['player_1_name'], True, False)
        
        # write workable index df to .csv file
...
def split_data_strat(df, target):
    ...
    Data split is stratified on the target variable specified.
    '''
    # import splitter
