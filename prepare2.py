...
def prepare_atp():
    ...
    # combine, drop duplicates, convert set to list type and sort for viewing 
    ...
    # loop through each pair of players for every record
    for players, winner in df.groupby(['player_1', 'player_2'])['winner']:
        # assign variable to length of records where there is a winner (all records)
        num_matches = len(winner)
        # set counters for h2h match wins for 
        p1 = 0
        p2 = 0
        # create an empty dataframe to fill with h2h stats
        temp_df = pd.DataFrame()
        # loop through matches
        for i in range(0, num_matches):
            # if player 1 is the winner,
            if winner.iloc[i] == players[0]:
                # increment h2h counter for player 1 by 1
                p1 = p1 + 1
                # add dataframe records which include both players, their h2h counters and an index comprised of the winner namae
                temp_df = temp_df.append(pd.DataFrame([[players[0], players[1], p1, p2]], columns = ['player_1', 'player_2', 'h2h_1', 'h2h_2'], index = [winner.index[i]]))
            # otherwise,
            else:
                # increment h2h counter for player 2 by 1
                p2 = p2 + 1 
                # add dataframe records which include both players, their h2h counter and an index comprised of the winner name
                temp_df = temp_df.append(pd.DataFrame([[players[0], players[1], p1, p2]], columns = ['player_1', 'player_2', 'h2h_1', 'h2h_2'], index = [winner.index[i]]))
